...
const Login = () => {
  ...
  useEffect(() => {
    google.accounts.id.initialize({
      client_id:
        '633767045331-hdief49o2fq3m6p5iiqh3h6brtf859v7.apps.googleusercontent.com',
      callback: handleCallBackResponse,
    });

    google.accounts.id.renderButton(document.getElementById('sigin'), {
      theme: 'outline',
      size: 'large',
    });
  }, []);
  ...
  function handleCallBackResponse(response) {
    ...
    csrf();
    axios
      .post('/api/login-google', { name, email, password })
      .then((response) => {
        console.log(response.data);
        navigate('/');
        localStorage.setItem('user', JSON.stringify(response.data.userDetails));
