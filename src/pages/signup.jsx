...
const Signup = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios
        .post('/api/register-user', { email, password, confirmPassword })
        .then((response) => {
        ...
        .catch(function (error) {
          if (error.response) {
            ...
            if (error.response.data.email?.[0] == 'Email is required')
              ...
              error.response.data.email?.[0] == 'Email address is invalid'
            )
              setError('Định dạng email không hợp lệ');
            else if (error.response.data.email?.[0] == 'Email is existed')
              setError('Email đã đăng ký');
            else if (
              error.response.data.password?.[0] == 'Password is required'
            )
              ...
              error.response.data.password?.[0] == 'Password is confirmed'
            )
